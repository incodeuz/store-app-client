...
const FormTemplate = ({ datas }) => {
  const { isData, setIsData } = useContext(AddStudentContext);

  const addStudent = (e) => {
    e.preventDefault();
    const [emailInp, fnameInp, lnameInp, phoneInp, idInp] =
  ...
  return (
    <form
      onSubmit={(e) => {
        datas === "teacher" ? addTeacher(e) : addStudent(e);
      }}
      className={`w-full mb-[40px] rounded-lg p-5 bg-slate-100 `}
    >
